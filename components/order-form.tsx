...
import { Loader2, Plus, ShoppingCart, Sparkles, Trash2 } from "lucide-react";
import { toast } from "sonner";
import type { DrainMode, MenuItem, PlaceOrderResult } from "@/lib/types";
...
export function OrderForm({ menuItems, balance, drainMode, orderDateLabel, canOrder, cutoffMessage }: OrderFormProps) {
  ...
  const canAfford = drainMode === "confirmation" ? orderTotal > 0 : balance >= orderTotal && orderTotal > 0;
  ...
  async function handlePlaceOrder() {
    ...
    try {
      ...
      toast.success(`Order placed! ${lines.length} item type(s), total ${Number(result.total).toLocaleString()} UGX`);
      setLines([]);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {lines.length > 0 && drainMode === "automatic" && !canAfford && (
